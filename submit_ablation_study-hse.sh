...
# 执行训练（完整参数列表，不使用数组）
python train_with_cross_modal_attention.py     --root_dir ${DATA_ROOT}     --dataset jarvis     --property ${property}     --train_ratio 0.8     --val_ratio 0.1     --test_ratio 0.1     --batch_size 64     --epochs 300     --learning_rate 5e-4     --weight_decay 1e-3     --warmup_steps 2000 --graph_builder kgcnn  --late_fusion_type tucker --late_fusion_output_dim 64 --middle_fusion_use_gate_norm True --middle_fusion_use_learnable_scale True --middle_fusion_initial_scale 12    --alignn_layers 5     --gcn_layers 5     --hidden_features 256     --graph_dropout 0.15     --use_cross_modal ${use_cross_modal}    --cross_modal_num_heads 2  --cross_modal_dropout 0.1   --use_middle_fusion ${use_middle}     --middle_fusion_layers 2     --use_fine_grained_attention ${use_fg}     --middle_fusion_dropout 0.35     --fine_grained_hidden_dim 256     --fine_grained_num_heads 8     --fine_grained_dropout 0.35     --fine_grained_use_projection ${use_proj}     --early_stopping_patience 50     --output_dir ${output_dir}     --num_workers 24     --random_seed ${seed}
