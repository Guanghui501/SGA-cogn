...
from models.cogn import coGNConfig
...
def get_parser():
    ...
    # 模型参数
    parser.add_argument('--alignn_layers', type=int, default=5,
                        help='coGN卷积层数（沿用原ALIGNN参数名以保持兼容）')
    parser.add_argument('--gcn_layers', type=int, default=5,
                        help='边特征MLP层数（沿用原GCN参数名以保持兼容）')
    parser.add_argument('--hidden_features', type=int, default=256,
                        help='隐藏层特征维度')
    parser.add_argument('--graph_dropout', type=float, default=0.0,
                        help='coGN/GCN层的dropout率（用于正则化）')
...
#        gcn_layers=args.gcn_layers,
    model_config = coGNConfig(
        name="cogn",
        cogn_layers=args.alignn_layers,
        node_dim=args.hidden_features,
        edge_dim=args.hidden_features,
        edge_mlp_layers=args.gcn_layers,
        node_mlp_layers=1,
...
def main():
    ...
    print(f"  边MLP层数(原GCN层参数): {args.gcn_layers}")
    ...
    # 提取模型配置（coGNConfig）用于保存到checkpoint
    model_config = config_dict['model']

    #保存配置（将 coGNConfig 对象转换为字典以便 JSON 序列化）
    ...
    try:
        supports_graph_builder = (
            inspect is not None
            and "graph_builder" in inspect.signature(get_train_val_loaders).parameters
        )
    except Exception:
        supports_graph_builder = False
    if not supports_graph_builder:
        loader_kwargs.pop("graph_builder", None)
